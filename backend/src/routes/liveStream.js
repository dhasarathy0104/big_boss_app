...
import { requireSupervisor, isSelfOrOwnEmployee } from '../auth.js';
...
// Any supervisor tier (GM/AGM/Manager/AM/TL) or superadmin asks to watch one
// employee's screen live — requireSupervisor only confirms the caller isn't
// an employee; isSelfOrOwnEmployee below does the actual hierarchy check
// (must be that employee's ancestor). This only creates the session record
// the WebSocket relay (see liveRelay.js) uses to authorize and pair the two
// sides — the video itself flows over that relay, not through any route
// here, and nothing is written to the database.
liveStreamRouter.post('/live-sessions', requireSupervisor, ah(async (req, res) => {
...
liveStreamRouter.get('/live-sessions/:id', requireSupervisor, ah(async (req, res) => {
...
liveStreamRouter.post('/live-sessions/:id/stop', requireSupervisor, ah(async (req, res) => {
  const session = getSession(req.params.id);
